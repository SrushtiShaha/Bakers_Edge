...
//       const token = localStorage.getItem("adminToken");
//       // UPDATED LINE
//       const res = await axios.get(`${API_URL}/api/admin/vendor/requests`, {
//         headers: { Authorization: `Bearer ${token}` },
//       });
//       setPendingRequests(res.data || []);
//     } catch (err) {
...
//         `${API_URL}/api/admin/vendor/approve/${selectedRequest._id}`,
//         { password },
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
//       toast.success("Vendor approved successfully! SMS and Email confirmation sent.");
...
//     try {
//       // This line was already correct
//       await axios.delete(`${API_URL}/api/admin/vendor/request/${id}`, {
//         headers: { Authorization: `Bearer ${token}` }
//       });
//       toast.info("Vendor request deleted.");
//       fetchPendingRequests();
...
//   // This function is likely for testing, but it's good practice to update it too.
//   const handleAdminLogin = async () => {
//     try {
//         // UPDATED LINE
...
const AdminDashboard = () => {
  ...
  const finalizeApprove = async () => {
    ...
    try {
      ...
      window.alert("Vendor approved successfully! SMS and Email confirmation sent.");
  ...
  return (
    <div className="admin-container d-flex" style={{ minHeight: "100vh" }}>
      ...
      <div className="content px-3 py-4" style={{ flex: 1 }}>
        ...
        {/* This is the first pop-up (Password Modal) */}
        {selectedRequest && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white p-6 rounded-xl shadow-lg w-96">
              <h3 className="text-xl font-bold mb-4">Set Password for {selectedRequest.name}</h3>
