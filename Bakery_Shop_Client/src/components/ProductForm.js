...
const ProductForm = () => {
  ...
  const sortProducts = (list) => [...list].sort((a, b) => a.name.localeCompare(b.name));
  ...
  const fetchProducts = async () => {
    ...
    try {
      const token = localStorage.getItem('vendorToken');
      const res = await axios.get(`/api/products?_t=${Date.now()}`, {
  ...
  useEffect(() => { fetchProducts(); }, []);
  useEffect(() => { products.forEach(p => generateBarcode(p._id)); }, [products]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { name, quantity, price, expiryDate, manufacturingDate } = form;

    if (!name.trim() || quantity <= 0 || price <= 0 || !expiryDate || !manufacturingDate) {
      toast.error("All fields including MFG and EXP dates are required.");
      return;
    }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        toast.warn("Product not found (may have expired or been deleted).");
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this product?")) {
      ...
      try {
        ...
        if (err.response?.status === 404) {
          toast.warn("This product no longer exists (it may have expired).");
        } else if (err.response?.status === 401) {
          toast.error("Unauthorized. Please log in again.");
          localStorage.removeItem('vendorToken');
          window.location.href = "/vendor-login";
  ...
  // ✅ Move expired products
  const handleMoveExpired = async () => {
    if (window.confirm("Archive all expired products?")) {
      const token = localStorage.getItem('vendorToken');
      try {
        const res = await axios.post(`/api/products/move-expired`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
        toast.success(res.data.message || "Expired products archived.");
        fetchProducts();
      } catch (err) {
        if (err.response?.status === 401) {
          toast.error("Unauthorized. Please log in again.");
          ...
          toast.error("Error archiving expired products.");
        }
      }
    }
  };
  ...
  const generateBarcode = (id) => {
    const canvas = barcodeRefs.current[id];
    if (canvas) JsBarcode(canvas, id.toString(), { format: "CODE128", displayValue: false, width: 1, height: 20 });
  };
  ...
  const generatePDFWithBarcodes = (product) => {
    ...
    pdf.text(`Price: ₹${product.price}`, 10, 30);
    if (product.manufacturingDate)
      pdf.text(`MFG: ${new Date(product.manufacturingDate).toLocaleDateString()}`, 10, 40);
    pdf.text(`Expiry Batches:`, 10, 50);
    let y = 60;
    if (product.expiryBatches?.length > 0) {
      product.expiryBatches.forEach(b => {
        if (y > 280) { pdf.addPage(); y = 20; }
        pdf.text(`- EXP: ${new Date(b.expiryDate).toLocaleDateString()} (Qty: ${b.quantity})`, 15, y);
        y += 10;
      });
    } else pdf.text("No expiry info.", 15, y);
    pdf.addImage(imgData, "PNG", 10, y + 5, 100, 30);
    pdf.save(`${product.name}_barcode.pdf`);
  };
  ...
  const getExpiryAlert = (batches = []) => {
    ...
    let expiredQty = 0, expToday = 0, expTomorrow = 0, expIn2Days = 0;
    batches.forEach(b => {
      const exp = new Date(b.expiryDate); exp.setHours(0, 0, 0, 0);
      if (exp < today) expiredQty += b.quantity;
      else if (exp.getTime() === today.getTime()) expToday += b.quantity;
      else if (exp.getTime() === tomorrow.getTime()) expTomorrow += b.quantity;
      else if (exp.getTime() === twoDaysLater.getTime()) expIn2Days += b.quantity;
    });

    if (expiredQty > 0) return <span style={{ color: "red", fontWeight: "bold" }}>Expired (Qty: {expiredQty})</span>;
    if (expToday > 0) return <span style={{ color: "orange", fontWeight: "bold" }}>Expires Today (Qty: {expToday})</span>;
    if (expTomorrow > 0) return <span style={{ color: "#e0a800", fontWeight: "bold" }}>Expires Tomorrow (Qty: {expTomorrow})</span>;
    if (expIn2Days > 0) return <span style={{ color: "blue", fontWeight: "bold" }}>Expires in 2 Days (Qty: {expIn2Days})</span>;
    return null;
  };

  const filtered = products.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase()));
  const isFormInvalid = !form.name.trim() || Number(form.quantity) <= 0 || Number(form.price) <= 0 || !form.expiryDate || !form.manufacturingDate;
  ...
  return (
    <div className="container mt-4">
      ...
      <form onSubmit={handleSubmit} className="p-4 border rounded shadow bg-white">
        <h5 className="mb-3 border-bottom pb-2 text-primary">Add New Product / Batch</h5>
        <div className="row g-4">
          <div className="col-md-4"><label>Product Name</label>
            <input type="text" name="name" className="form-control" value={form.name} onChange={handleChange} required />
          </div>
          <div className="col-md-4"><label>Batch Quantity</label>
          ...
          <div className="col-md-4"><label>Weight</label>
            <input type="text" name="weight" className="form-control" value={form.weight} onChange={handleChange} />
          </div>
          <div className="col-md-4"><label>Manufacturing Date</label>
            <input type="date" name="manufacturingDate" className="form-control" value={form.manufacturingDate} onChange={handleChange} required />
          </div>
          <div className="col-md-4"><label>Batch Expiry Date</label>
            <input type="date" name="expiryDate" className="form-control" value={form.expiryDate} onChange={handleChange} required />
          </div>
          <div className="col-12 text-end">
            <button className="btn btn-success px-4" type="submit" disabled={isFormInvalid}>Save Batch</button>
          </div>
        </div>
      </form>
      ...
      <div className="d-flex justify-content-between align-items-center mt-5">
        <h3 className="mb-3">📦 Product List</h3>
        <button className="btn btn-danger" onClick={handleMoveExpired}>Archive Expired Products</button>
      </div>
      ...
      {loading ? (
        ...
        <table className="table table-bordered table-hover">
          <thead className="table-dark">
            <tr>
              <th>Name</th><th>Total Qty</th><th>Price</th><th>Weight</th>
              <th>Barcode</th><th>Expiry Batches</th><th>Alert</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(p => (
      ...
      {/* Edit Modal */}
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        <Modal.Header closeButton className="bg-primary text-white"><Modal.Title>Edit Product</Modal.Title></Modal.Header>
        <Modal.Body>
          {["name", "price", "weight", "manufacturingDate"].map(field => (
