...
const PendingApprovals = () => {
  ...
  const [selectedRequest, setSelectedRequest] = useState(null);
  const [password, setPassword] = useState("");
  const [isApproving, setIsApproving] = useState(false); // <-- 1. ADD THIS STATE
  ...
  const handleApprove = async () => {
    ...
    setIsApproving(true); // <-- 3. SET LOADING STATE
    ...
    try {
      ...
      if (err.response && err.response.status === 404) {
        toast.error("This request was already processed or not found.");
      } else {
        toast.error("Failed to approve vendor!");
      }
    } finally {
      setIsApproving(false); // <-- 5. RESET LOADING STATE (ALWAYS)
    }
  };
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-96">
            ...
            <input
              ...
              onChange={(e) => setPassword(e.target.value)}
              className="w-full p-2 border rounded mb-4"
              disabled={isApproving} // <-- 6. DISABLE INPUT
            />
            <div className="flex justify-end space-x-3">
              <button
                onClick={() => setSelectedRequest(null)}
                className="px-4 py-2 bg-gray-400 text-white rounded-lg"
                disabled={isApproving} // <-- 7. DISABLE CANCEL
              >
                Cancel
              </button>
              <button
                onClick={handleApprove}
                // <-- 8. UPDATE BUTTON STYLE AND TEXT
                className={`px-4 py-2 text-white rounded-lg ${
                  isApproving
                    ? "bg-green-300"
                    : "bg-green-600 hover:bg-green-700"
                }`}
                disabled={isApproving} // <-- 9. DISABLE BUTTON
              >
