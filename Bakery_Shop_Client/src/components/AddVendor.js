...
function AddVendor({ onFormSubmitSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Get the base URL from .env and append the specific endpoint
    const API_URL = `${process.env.REACT_APP_API_URL}/api/vendor/request`;

    try {
      await axios.post(
        API_URL, // Use the new environment-safe URL
        formData
      );
